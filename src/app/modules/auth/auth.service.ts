...
const forgotPass = async (payload: { id: string }) => {
  ...
  else if (user.role === ENUM_USER_ROLE.PRIVATECUSTOMER) {
    profile = await PrivateCustomer.findOne({ id: user.id })
  }
  else if (user.role === ENUM_USER_ROLE.BUSINESSCUSTOMER) {
    profile = await BusinessCustomer.findOne({ id: user.id })
  }
